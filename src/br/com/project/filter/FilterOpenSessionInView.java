...
public class FilterOpenSessionInView extends DelegatingFilterProxy implements
	...
	public void doFilter(ServletRequest servletRequest,
		...
		try {
			...
			servletRequest.setCharacterEncoding("UTF-8"); // Define codificação 
			...
			chain.doFilter(servletRequest, servletResponse); // executa nossa ação no servidor 
			// após de executar ação (Resposta)

			// JDBC Spring - commit
			transactionManager.commit(status);

			// transação do hibernerte - executa os sql (flush) e commita
